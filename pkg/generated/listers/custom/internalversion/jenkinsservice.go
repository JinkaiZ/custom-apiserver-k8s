...
type JenkinsServiceLister interface {
	...
	List(selector labels.Selector) (ret []*custom.JenkinsService, err error)
...
// List lists all JenkinsServices in the indexer.
func (s *jenkinsServiceLister) List(selector labels.Selector) (ret []*custom.JenkinsService, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*custom.JenkinsService))
	})
	return ret, err
}
...
type JenkinsServiceNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*custom.JenkinsService, error)
...
// List lists all JenkinsServices in the indexer for a given namespace.
func (s jenkinsServiceNamespaceLister) List(selector labels.Selector) (ret []*custom.JenkinsService, err error) {
...
// Get retrieves the JenkinsService from the indexer for a given namespace and name.
func (s jenkinsServiceNamespaceLister) Get(name string) (*custom.JenkinsService, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(custom.Resource("jenkinsservice"), name)
	}
	return obj.(*custom.JenkinsService), nil
}
