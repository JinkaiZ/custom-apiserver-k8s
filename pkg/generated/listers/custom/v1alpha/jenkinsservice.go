...
type JenkinsServiceLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha.JenkinsService, err error)
...
// List lists all JenkinsServices in the indexer.
func (s *jenkinsServiceLister) List(selector labels.Selector) (ret []*v1alpha.JenkinsService, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha.JenkinsService))
	})
	return ret, err
}
...
type JenkinsServiceNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha.JenkinsService, error)
...
// List lists all JenkinsServices in the indexer for a given namespace.
func (s jenkinsServiceNamespaceLister) List(selector labels.Selector) (ret []*v1alpha.JenkinsService, err error) {
...
// Get retrieves the JenkinsService from the indexer for a given namespace and name.
func (s jenkinsServiceNamespaceLister) Get(name string) (*v1alpha.JenkinsService, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha.Resource("jenkinsservice"), name)
	}
	return obj.(*v1alpha.JenkinsService), nil
}
